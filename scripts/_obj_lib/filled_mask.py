...
def refine_masks_to_original_edges(
    masks: List[np.ndarray],
    original_bgr: np.ndarray,
    *,
    max_shift: int = 8,
    min_improvement: float = 0.35,
) -> Tuple[List[np.ndarray], List[dict]]:
    """Locally translate mapped masks toward strong original-image edges.

    Global registration cannot model image-generator local drift.  For every
    instance, search a small integer translation and minimize the symmetric
    distance from its boundary to Canny edges.  Accept a shift only when it
    improves the score enough; this prevents texture-driven gratuitous moves.
    """
    ...
    for mask in masks:
        ...
        boundary = base ^ cv2.erode(base, kernel, iterations=1)
        ys, xs = np.where(boundary > 0)
        if xs.size < 6:
            refined.append(base)
            diagnostics.append({"dx": 0, "dy": 0, "before": 999.0, "after": 999.0})
            continue

        def score(dx: int, dy: int) -> float:
            xx, yy = xs + dx, ys + dy
            valid = (xx >= 0) & (xx < width) & (yy >= 0) & (yy < height)
            if int(valid.sum()) < max(6, int(0.90 * xs.size)):
                return 999.0
            values = distance[yy[valid], xx[valid]].astype(np.float64, copy=False)
            # Robust mean: ignore the worst 10% of boundary samples.
            cutoff = max(1, int(np.ceil(0.90 * values.size)))
            return float(np.partition(values, cutoff - 1)[:cutoff].mean())
        ...
        after, dx, dy = best
        if before - after < float(min_improvement):
            dx = dy = 0
            after = before
        transform = np.array([[1.0, 0.0, dx], [0.0, 1.0, dy]], np.float32)
        shifted = cv2.warpAffine(
            base, transform, (width, height), flags=cv2.INTER_NEAREST,
            borderMode=cv2.BORDER_CONSTANT, borderValue=0,
        )
        refined.append((shifted > 0).astype(np.uint8))
        diagnostics.append({
            "dx": int(dx), "dy": int(dy),
            "before": float(before), "after": float(after),
            "at_search_limit": bool(
                (dx != 0 or dy != 0)
                and (abs(int(dx)) == int(max_shift) or abs(int(dy)) == int(max_shift))
            ),
        })

    refined.sort(key=_mask_sort_key)
